"""Test: co przeglądarka wysyła w payment (po PUT z pustymi komponentami).

Łapiemy payment request body + response żeby zobaczyć czego brakuje w naszym
payment (error 114). Przechodzimy cały flow: conversations -> build -> PUT -> payment.
"""
...
OUT = ROOT / "output" / f"payment_body_{int(time.time())}.json"


def main() -> int:
    wynik = {"payment_body": None, "payment_resp": None, "checkout_put": None, "err": None}
    try:
        with Camoufox(persistent_context=True, headless=False, user_data_dir=PROFIL,
            ...
            def on_request(req):
                u = req.url
                try:
                    if "/checkout/payment" in u:
                        wynik["payment_body"] = req.post_data
                    elif u.endswith("/checkout") and req.method == "PUT":
                        wynik["checkout_put"] = req.post_data
                except Exception:
                    pass

            def on_response(resp):
                try:
                    if "/checkout/payment" in resp.url:
                        wynik["payment_resp"] = {"status": resp.status, "body": resp.text()[:1500]}
                except Exception:
                    pass
            ...
            try:
                ...
                try:
                    ...
                    wynik["err"] = f"klik: {e}"
                    return 1
            # Poczekaj na stronę checkout i kliknij "Zapłać" / "Kup".
            page.wait_for_timeout(5000)
            for txt in ["Zapłać", "Kup", "Potwierdź", "Pay", "Confirm"]:
                try:
                    page.click(f"button:has-text('{txt}')", timeout=3000)
                    wynik["kliknieto_platnosc"] = txt
                    break
                except Exception:
                    continue
