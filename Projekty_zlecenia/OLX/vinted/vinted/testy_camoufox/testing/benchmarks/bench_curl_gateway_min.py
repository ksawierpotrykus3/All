# coding: utf-8
"""Minimalny flow zakupowy do BRAMKI — scala PUT-y w jeden pełny components.

Hipoteza (z kodu frontendu, chunk 0m6z-r2 / 131~8x): `updateSingleCheckoutData` wysyła
`PUT /purchases/{id}/checkout` z PEŁNYM obiektem `components` (payment_method +
shipping_pickup_options + shipping_pickup_details naraz). Frontend rozbija to na 6 PUT-ów
tylko z powodu kroków UI. Test: czy serwer przyjmie JEDEN PUT z pełnym components.

Minimalna sekwencja (5 żądań checkout, zamiast 8):
  conversations -> build -> GET pickup_points -> 1x PUT (pełny components) -> payment

Bez Camoufox, bez selektorów. Screenshot bramki z timestampem ms (render pomocniczy).
"""
import json
import time
...
def main():
    ...
    # 1) catalog (scrapowanie — poza ścieżką checkout)
    ...
    # --- POCZĄTEK ŚCIEŻKI CHECKOUT (5 żądań) ---
    t_checkout = now_ms()

    # 2) conversations -> transaction_id
    ...
    # 4) pickup_points -> point_code (nie do pominięcia, sekcja 32.4)
    point = None
    if so_id:
        ...
        step("pickup_points", http=r.status_code, n=len(spoints), point_code=point.get("code"))

    # 5) JEDEN PUT z pełnym components (zamiast 4-6 osobnych PUT-ów)
    details = {}
    if rate_uuid:
    ...
    try:
        ch_put = _find_checksum(r.json())
    except Exception:
        ch_put = []
    step("PUT (pełny components)", http=r.status_code, checksum=bool(ch_put))

    # 6) payment -> bramka
    checksum = (ch_put or ch_build)[0] if (ch_put or ch_build) else ""
...
def _save():
    out = BASE_DIR / "wynik_bench_gateway_min.json"
