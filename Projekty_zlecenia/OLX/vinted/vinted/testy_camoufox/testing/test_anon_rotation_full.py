# coding: utf-8
"""Test: czy rotacja anon_id wystarczy do pelnego flow (conversations -> build -> payment)?"""
import json
import uuid
...
def full_flow_with_anon(anon_id, label):
    """Pelny flow z konkretnym anon_id."""
    s = cr.Session()
    ...
    if not checkout_id:
        print(f"[{label}] build FAIL: {r.status_code}")
        return False
    ...
    if r.status_code != 200:
        print(f"[{label}] PUT payment_method FAIL: {r.status_code}")
        return False
    ...
    if r.status_code != 200:
        print(f"[{label}] PUT pickup_details FAIL: {r.status_code}")
        return False
    ...
    print(f"[{label}] payment: {r.status_code} status={pay_status}")
    return r.status_code == 200 and pay_status == "pending"

# Test 3 flow z roznymi anon_id
for i in range(3):
    anon = str(uuid.uuid4())
    print(f"\n=== FLOW {i+1} (anon_id={anon[:8]}...) ===")
    ok = full_flow_with_anon(anon, f"flow{i+1}")
    print(f"Wynik: {'SUKCES' if ok else 'FAIL'}")
