# coding: utf-8
"""Test v7 - pelny flow: build -> PUT payment_method 12 -> GET nearby_pickup_points ->
wybierz shipping_point -> PUT pickup_details {rate_uuid, point_code, point_uuid}
-> payment (poprawny checksum, bez incognia) -> status.

Klucz: odpowiedz pickup_points ma 'shipping_points' (nie 'pickup_points').
"""
import json
import time
...
# 5) GET nearby pickup points -> shipping_points
point = None
if so_id:
    ...
    if isinstance(pp, dict):
        result["top_keys"] = list(pp.keys())
        result["suggested"] = pp.get("suggested_shipping_point_code")
        spoints = pp.get("shipping_points") or []
        result["n_points"] = len(spoints)
        for sp in spoints[:3]:
            sp_inner = sp.get("point", {})
            print(f"  POINT: code={sp_inner.get('code')} rate_uuid={sp_inner.get('rate_uuid')} "
                  f"name={sp_inner.get('name')} addr={sp_inner.get('address_line1')}")
        # wybierz punkt: sugerowany albo pierwszy pasujacy do rate_uuid
        if spoints:
            sug = pp.get("suggested_shipping_point_code")
            for cand in spoints:
            ...
            result["chosen"] = {"code": point.get("code"), "uuid": point.get("uuid"),
                                "rate_uuid": point.get("rate_uuid"), "name": point.get("name")}
    results["pickup_points"] = result
    time.sleep(0.3)
else:
...
if point:
    ...
    # spójnosc: rate_uuid z punktu ma pierwszenstwo (build rate mogl byc inny niz rate punktu)
...
try:
    pj = r.json()
    ch3 = find_checksum(pj)
    body600 = r.text[:600]
except Exception:  # noqa: BLE001
    ch3, body600 = [], r.text[:600]
...
(BASE_DIR / "wynik_reservation_test_v7.json").write_text(
    json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
print("\nZapisano: wynik_reservation_test_v7.json")
