# coding: utf-8
"""Reset DataDome przez pelna sesje Camoufox: strona itemu + klik 'Kup teraz' +
przejscie checkoutu w przegladarce. Potem test: czy curl_cffi odzyskuje dostep
do checkout/build.
"""
import json
import time
...
with Camoufox(persistent_context=True, headless=True,
              ...
              fingerprint_preset=False, humanize=False) as ctx:
    page = ctx.new_page()
    # 1) strona itemu
    page.goto("https://www.vinted.pl/items/9807925466-genesis-krypton-700",
              wait_until="domcontentloaded", timeout=60000)
    time.sleep(3)
    # 2) klik 'Kup teraz' (wyzwala checkout w przegladarce)
    try:
    ...
    # 3) poczekaj na checkout page (moze byc w nowej karcie)
    time.sleep(5)
    cookies = ctx.cookies()
    print(f"Camoufox po checkout: {len(cookies)} cookies")
    results["camoufox_cookies"] = len(cookies)

# 4) test curl_cffi po reset
s = cr.Session()
for c in cookies:
    try:
...
results["build_http"] = r.status_code
...
(BASE_DIR / "wynik_reset_datadome.json").write_text(
    json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
print("Zapisano: wynik_reset_datadome.json")
