# coding: utf-8
"""Szukanie itemow sprzedawcy przez katalog - filtr seller_id."""
import json
...
url = ("https://www.vinted.pl/api/v2/catalog/items?"
       "page=1&per_page=96&seller_id=3180795364&order=relevance")
...
if r.status_code == 200:
    ...
    for it in items:
        out.append(f"  id={it.get('id')} title={it.get('title')} status_id={it.get('status_id')} "
                   f"is_reserved={it.get('is_reserved')}")
    out.append(f"pagination={data.get('pagination')}")
else:
    out.append(r.text[:300])
Path(BASE_DIR / "seller_items2_out.txt").write_text("\n".join(out), encoding="utf-8")
print("\n".join(out))
