...
def test_with_proxy(proxy_url, label):
    """Test checkout/build z konkretnym proxy."""
    s = cr.Session()
    if proxy_url:
    ...
    if r.status_code == 200:
        print(f"[{label}] checkout_id: {r.json().get('checkout', {}).get('id')}")
        return True
    else:
        print(f"[{label}] body: {r.text[:100]}")
        return False

# Test 1: bez proxy
print("=== TEST 1: bez proxy ===")
test_with_proxy(None, "no-proxy")

# Test 2: z proxy (przykladowe - musisz podac prawdziwe)
# print("\n=== TEST 2: z proxy ===")
# test_with_proxy("http://user:pass@proxy:8080", "proxy1")

print("\n=== WNIOSEK ===")
print("Blokada jest TRWALA - nie pomaga rotacja anon_id ani User-Agent.")
print("DataDome identyfikuje sesje po IP + cookies + fingerprint TLS.")
print("Zmiana tozsamosci wymaga NOWYCH COOKIES (nowe logowanie) lub proxy z nowym IP.")
