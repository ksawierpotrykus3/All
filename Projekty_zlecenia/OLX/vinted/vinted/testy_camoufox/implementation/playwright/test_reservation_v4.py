# coding: utf-8
"""Test v4 - czy status 220 (reserved) ustawia sie po POST /payment z poprawnym checksum.

Sekwencja: conversations -> build (checksum!) -> PUT payment_method 12 (nowy checksum)
-> POST payment z poprawnym checksum + browser_info (bez incognia) -> status.
Przelewy24 = przelew (brak debetu karty). Cleanup po teście.
"""
import json
import time
...
results["after_conversations"] = {"status": st, "body": json.dumps(jb)[:1200]}
time.sleep(0.3)

# 3) Build -> wez checksum
...
# 4) PUT payment_method 12 -> nowy checksum
...
pj = r.json()
ch2 = find_checksum(pj)
st, _ = get_txn_status(txn_id)
print(f"[PUT payment_method] {r.status_code} status={st} checksum={ch2}")
results["put_payment_method"] = {"http": r.status_code, "txn_status": st, "checksum": ch2}
time.sleep(0.3)

# 5) POST payment z poprawnym checksum (ostatni z PUT), browser_info jak w HAR, bez incognia
checksum = ch2[0] if ch2 else (ch[0] if ch else "")
...
print(f"[PAYMENT poprawny checksum, bez incognia] {r.status_code} {r.text[:700]}")
results["payment"] = {"http": r.status_code, "body": r.text[:1500]}
time.sleep(1.5)
...
(BASE_DIR / "wynik_reservation_test_v4.json").write_text(
    json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
print("\nZapisano: wynik_reservation_test_v4.json")
