# coding: utf-8
"""Reset DataDome przez Playwright Firefox (standardowy, bez Camoufox).
Potem test: czy curl_cffi odzyskuje dostep do checkout/build.
"""
import json
import time
...
with sync_playwright() as p:
    ...
    time.sleep(5)
    cookies = ctx.cookies()
    print(f"Playwright po checkout: {len(cookies)} cookies")
    browser.close()
...
(BASE_DIR / "wynik_reset_playwright.json").write_text(
    json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
print("Zapisano: wynik_reset_playwright.json")
