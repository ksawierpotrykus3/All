# coding: utf-8
"""Hybryda v2: Camoufox wchodzi NA STRONE ITEMU (wyzwala requesty api), potem curl_cffi checkout."""
import json
import time
...
with Camoufox(persistent_context=True, headless=True,
    ...
    time.sleep(4)
    # scroll, zeby wyzwolic lazy requesty
    page.mouse.wheel(0, 500)
    time.sleep(2)
    cookies = ctx.cookies()
    print(f"Camoufox po item page: {len(cookies)} cookies")
...
(BASE_DIR / "wynik_hybryda_v2.json").write_text(
    json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
print("Zapisano: wynik_hybryda_v2.json")
