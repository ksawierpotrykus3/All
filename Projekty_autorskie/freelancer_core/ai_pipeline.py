...
class MockAIPipeline(BaseAIPipeline):
    ...
    def generate_proposal(self, job_detail: Dict[str, Any]) -> ProposalResult:
        ...
        # Budzet moze byc zakresem "1500 - 12500 INR" -> bierzemy dolna granice.
        kwota = 1500
        raw = job_detail.get("budget_raw") or {}
        if isinstance(raw, dict) and (raw.get("minimum") or raw.get("maximum")):
            try:
                kwota = int(float(raw.get("minimum") or raw.get("maximum")))
            except (TypeError, ValueError):
                pass
        else:
            budget_str = str(job_detail.get("budget", "1500"))
            m = re.search(r"(\d[\d\s]*)\s*[-–]", budget_str)
            if m:
                kwota = int(re.sub(r"\D", "", m.group(1)) or 1500)
            else:
...
class SlotChainAIPipeline(BaseAIPipeline):
    ...
    def generate_proposal(self, job_detail: Dict[str, Any]) -> ProposalResult:
        ...
        wynik = self._run_chain(
            f"freelancer-job-{job_id}",
            job_detail,
            parent_id="freelancer-bot",
