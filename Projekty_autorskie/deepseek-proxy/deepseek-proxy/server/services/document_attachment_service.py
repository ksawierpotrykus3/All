"""server/services/document_attachment_service.py — Document Attachment Strategy.
...
# Threshold of prompt length to trigger attachment extraction (same as prompt chunker threshold)
...
def extract_oversized_blocks_to_attachments(
    ...
    # ─────────────────────────────────────────────────────────────────────────
    if len(current_prompt) > threshold and len(attachments) < MAX_ATTACHMENTS_PER_REQUEST:
        # Check if prompt contains prior turns: from first [Assistant]: to last [User]:
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # 4. Final fallback pass: if prompt is STILL oversized (raw monolith without recognized tags)
    # ─────────────────────────────────────────────────────────────────────────
    if len(current_prompt) > threshold and len(attachments) < MAX_ATTACHMENTS_PER_REQUEST:
...
def format_history_to_markdown(history: list[dict]) -> str:
    ...
    Preserves 100% of the conversation turns without omitting any turn.
    For older turns (>15 turns ago), compacts oversized outputs to prevent OOM.
    """
...
def create_history_attachments(
    history: list[dict],
    max_file_size: int = MAX_DOCUMENT_SIZE_BYTES,
) -> list[DocumentAttachment]:
    """Formats history messages and packs them into DocumentAttachment objects (up to 100 MB per file)."""
    if not history:
        return []

    md_content = format_history_to_markdown(history)
    data_bytes = md_content.encode("utf-8")
    chunks = _split_bytes_to_chunks(data_bytes, chunk_size=max_file_size)

    attachments: list[DocumentAttachment] = []
    for p_idx, p_bytes in enumerate(chunks):
        if len(attachments) >= MAX_ATTACHMENTS_PER_REQUEST:
            break
        fname = (
            f"conversation_history_part{p_idx + 1}.md"
